#!/usr/bin/env python3
"""
Post-Processing CLIP-Style Keypoint Detection Model Training

This script implements post-training for the CLIP heatmap model using bedsheet data.
...
DEFAULT_CONFIG = {
    ...
    "keypoints_data_srcs": [
        ...
        "via_proj/via_project_01Sep2025_09h52m16s.json"
    ],
    ...
    "lr_gamma": 0.5,
    "warmup_epochs": 5,
    "model_save_path": "models/clip_style_bedsheet_post.pth",
    "save_interval": 10,
    "results_dir": "results_clip_bedsheet_post",
    ...
    "use_text_prior": True,
    "prior_prompts": [
        "bedsheet corner points",
        "sheet corner edges",
        "bedding corner junctions",
        "fabric corner points",
        "white sheet corners",
        "bedsheet edge corners"
    ],
    "negative_prompts": [
        "bed frame corners",
        "mattress edges",
...
class BedsheetAugmentation:
    """Data augmentation for bedsheet keypoint detection using heatmap-based spatial transformations."""
...
def generate_bedsheet_dataset_data(
    ...
    """
    Generate bedsheet dataset data using functional approach from multiple data sources.
    ...
    for keypoints_data_src, image_path in zip(keypoints_data_srcs, image_paths):
        print(f"Processing bedsheet data source: {keypoints_data_src} with images from: {image_path}")
        ...
        for filename in os.listdir(image_path):
            ...
            # # Apply YOLO masking on the resized images (if needed for bedsheet data)
            # try:
            #     from src.utils import extract_mask_compare
            #     mask = extract_mask_compare(img, yolo_model_finetuned, allowed_classes)
            #     if np.sum(mask) == 0:
            #         # If no mask found, use the whole image
            #         mask = np.ones_like(img[:,:,0])
            #     # Apply mask to both images
            #     img[mask == 0] = 0
            #     color_img[mask == 0] = 0
            # except ImportError:
            #     # If YOLO masking not available, use whole image
            #     print("Warning: YOLO masking not available, using whole image")
            #     mask = np.ones_like(img[:,:,0])
    ...
    print(f"Combined bedsheet dataset from {len(keypoints_data_srcs)} sources: {len(img_arr)} samples")
    return (
        np.array(img_arr),
...
class BedsheetKeypointDataset(Dataset):
    """Dataset for bedsheet keypoint detection with CLIP preprocessing."""
...
def collate_bedsheet_batch(batch):
    """Collate function for bedsheet dataset."""
...
def train_model_post(
    ...
    """
    Post-train the CLIP model on bedsheet data.
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(test_loader, desc="Evaluating")):
            ...
            for i, (pred_heat, gt_heat, img_id, orig_size) in enumerate(zip(
                ...
                result_path = os.path.join(results_dir, f'{name_without_ext}_post_clip_keypoints.png')
...
def convert_clip_model_to_tensorrt(
    model: ClipHeatmapModel, 
    model_path: str, 
    config: Dict[str, Any]
) -> Optional[str]:
    """
    Convert CLIP heatmap model to TensorRT format.
    ...
    try:
        print("Starting CLIP model TensorRT conversion...")
        ...
        # Convert model to ONNX first (simplified approach)
        ...
        # Export to ONNX with simplified settings
        ...
        try:
            torch.onnx.export(
                ...
                do_constant_folding=False,  # Disable constant folding to avoid device issues
        ...
        with open(tensorrt_path, 'wb') as f:
            if hasattr(engine, 'serialize'):
                # Older API - engine object with serialize method
                f.write(engine.serialize())
            else:
                # Newer API - engine is already serialized data
                f.write(engine)
...
def benchmark_clip_tensorrt_vs_pytorch(
    pytorch_model: ClipHeatmapModel,
    tensorrt_path: str,
    config: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Benchmark CLIP TensorRT vs PyTorch inference performance.
    
    Args:
        pytorch_model: Trained PyTorch model
        tensorrt_path: Path to TensorRT model
        config: Configuration dictionary
    ...
    try:
        print("Running CLIP TensorRT vs PyTorch benchmark...")
        
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        input_shape = (1, 3, config["image_size"], config["image_size"])
        num_runs = config.get("tensorrt_num_runs", 100)
        ...
        # Allocate buffers - handle both old and new TensorRT APIs
        try:
            # Check if this is newer TensorRT API (no get_binding_shape method)
            if hasattr(engine, 'get_binding_shape'):
                ...
                d_input = torch.cuda.FloatTensor(input_size)
                d_output = torch.cuda.FloatTensor(output_size)
                
                # Warmup TensorRT (older API)
                for _ in range(10):
                    context.execute(batch_size=1, bindings=[d_input.data_ptr(), d_output.data_ptr()])
            else:
                # Newer TensorRT API - use different approach
                print("Using newer TensorRT API for benchmarking...")
                # For newer API, we need to use different buffer allocation
                # This is a simplified approach - may need adjustment based on specific TensorRT version
                input_size = 1 * 3 * config["image_size"] * config["image_size"]
                output_size = 1 * 1 * config["image_size"] * config["image_size"]
                
                d_input = torch.cuda.FloatTensor(input_size)
                d_output = torch.cuda.FloatTensor(output_size)
                
                # Warmup TensorRT (newer API)
                for _ in range(10):
                    context.execute_v2([d_input.data_ptr(), d_output.data_ptr()])
        except Exception as e:
            print(f"TensorRT API compatibility issue: {e}")
            print("Skipping TensorRT benchmarking due to API incompatibility")
            return {}
        ...
        tensorrt_avg_time = (tensorrt_time / num_runs) * 1000  # Convert to ms
        ...
        results = {
            'pytorch': {
                ...
                'fps': 1000 / pytorch_avg_time
            },
            'tensorrt': {
                'total_time': tensorrt_time,
                'avg_inference_time': tensorrt_avg_time,
                'fps': 1000 / tensorrt_avg_time
            },
            'speedup': speedup,
            'num_runs': num_runs,
        ...
        print(f"  PyTorch avg time: {pytorch_avg_time:.2f} ms")
        print(f"  TensorRT avg time: {tensorrt_avg_time:.2f} ms")
        print(f"  Speedup: {speedup:.2f}x")
        print(f"  PyTorch FPS: {results['pytorch']['fps']:.1f}")
        print(f"  TensorRT FPS: {results['tensorrt']['fps']:.1f}")
...
def main_post_training_pipeline(config: Dict[str, Any]) -> Tuple[ClipHeatmapModel, Dict]:
    """
    Main post-training pipeline for CLIP model on bedsheet data.
    ...
    Returns:
        Tuple of (trained_model, training_history)
    """
    print("=== Post-Training CLIP-Style Keypoint Detection Model on Bedsheet Data ===")
    ...
    if config.get("use_augmentation", False):
        ...
        augmentation_transform = BedsheetAugmentation(config["image_size"])
    ...
    train_loader = DataLoader(
        ...
        shuffle=True, 
        pin_memory=True,
        collate_fn=collate_bedsheet_batch
    )
    val_loader = DataLoader(
        val_dataset, 
        batch_size=config["batch_size"], 
    ...
    if config.get("enable_tensorrt_conversion", False):
        print("\n" + "="*50)
        print("TENSORRT CONVERSION AND BENCHMARKING")
        print("="*50)
        
        # Convert model to TensorRT
        tensorrt_path = convert_clip_model_to_tensorrt(trained_model, config['model_save_path'], config)
        ...
        if tensorrt_path:
            ...
            if config.get("tensorrt_benchmark", False):
                ...
                benchmark_results = benchmark_clip_tensorrt_vs_pytorch(
                    trained_model, tensorrt_path, config
                )
                ...
                if benchmark_results:
                    ...
                    # Print summary
                    print(f"\nTensorRT Performance Summary:")
                    print(f"  Speedup: {benchmark_results['speedup']:.2f}x")
                    print(f"  PyTorch FPS: {benchmark_results['pytorch']['fps']:.1f}")
                    print(f"  TensorRT FPS: {benchmark_results['tensorrt']['fps']:.1f}")
                    print(f"  Input shape: {benchmark_results['input_shape']}")
                    print(f"  Benchmark runs: {benchmark_results['num_runs']}")
        else:
...
def test_tensorrt_functions():
    """Test TensorRT functions without full training"""
    print("Testing TensorRT functions...")
    
    # Create a simple test model
    config = DEFAULT_CONFIG.copy()
    config["enable_tensorrt_conversion"] = True
    config["tensorrt_benchmark"] = True
    config["tensorrt_num_runs"] = 10  # Fewer runs for testing
    ...
    # Save test model
    test_model_path = "test_clip_model.pth"
    torch.save(model.state_dict(), test_model_path)
    
    try:
        # Test TensorRT conversion
        tensorrt_path = convert_clip_model_to_tensorrt(model, test_model_path, config)
        
        if tensorrt_path:
            print("TensorRT conversion test successful!")
            
            # Test benchmarking
            benchmark_results = benchmark_clip_tensorrt_vs_pytorch(model, tensorrt_path, config)
            if benchmark_results:
                print("TensorRT benchmarking test successful!")
                print(f"Speedup: {benchmark_results['speedup']:.2f}x")
            else:
                print("TensorRT benchmarking test failed")
        else:
            print("TensorRT conversion test failed")
    
    finally:
        # Clean up test files
        if os.path.exists(test_model_path):
            os.remove(test_model_path)
        if os.path.exists(test_model_path.replace('.pth', '.trt')):
            os.remove(test_model_path.replace('.pth', '.trt'))

if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        test_tensorrt_functions()
    else:
        main()
